...
import { LogOut, Terminal, ChevronDown, Command, Menu, X } from "lucide-react";
...
interface AppShellProps {
  ...
  user: { email: string; tenantName?: string; role: string };
  navGroups: NavGroup[];
  logoLetter: string;
  portalLabel: string;
  accentColor: string;
...
export function AppShell({
  ...
  if (variant === "ide") {
    return (
      <div className="flex flex-col h-screen overflow-hidden bg-slate-950 text-slate-300">
        <div className="h-12 border-b border-slate-800 bg-slate-900 flex items-center justify-between px-4 shrink-0">
          <Link href={`/${role.toLowerCase()}/dashboard`} className="text-xs font-semibold text-slate-400 hover:text-white transition-colors">
            ← Exit Lab
          </Link>
          <div className="font-serif font-bold text-lg text-slate-100 tracking-tight">University LMS</div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {allItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={`${active ? 'text-blue-600' : 'opacity-60'}`}>{item.icon}</span>
                    {item.label}
                  </Link>
                );
              })}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-100 to-blue-50 border border-blue-200 flex items-center justify-center text-blue-800 font-bold text-xs uppercase shadow-inner">
                    {user.email.charAt(0)}
                  </div>
                  ...
                  {profileOpen && (
                    <>
                      ...
                      <motion.div 
                        ...
                        className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-200/60 z-50 overflow-hidden"
                      >
                        <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                          <p className="text-sm font-bold text-slate-900 truncate">{user.email}</p>
                          {user.tenantName && <p className="text-xs text-slate-500 mt-0.5 truncate font-medium">{user.tenantName}</p>}
                        </div>
                        <div className="p-2">
                          <button 
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="md:hidden border-b border-slate-200 bg-white overflow-hidden shadow-lg absolute w-full z-30"
          >
            <nav className="p-4 space-y-1">
              {allItems.map((item) => (
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-blue-800"
                >
      ...
      {/* Page Content */}
      <main className="flex-1 flex flex-col p-6 lg:p-10 w-full max-w-[1400px] mx-auto">
        {/* Academic Breadcrumbs */}
        {breadcrumbs && breadcrumbs.length > 0 && (
          <div className="w-full mb-8">
            <nav className="flex items-center gap-2 text-xs font-bold text-slate-400 tracking-widest uppercase">
